...
CREATE TABLE attendance_records (
    ...
    CONSTRAINT ck_attendance_records_status CHECK (attendance_status IN ('Present', 'Absent', 'Leave'))
);
...
CREATE TABLE staff_attendance_records (
    ...
    CONSTRAINT ck_staff_attendance_records_status CHECK (attendance_status IN ('Present', 'Absent', 'Leave'))
);
...
CREATE TABLE leave_requests (
    ...
    withdrawal_decided_at TIMESTAMPTZ NULL,
    withdrawal_decider_id UUID NULL REFERENCES users(id),
    withdrawal_remarks TEXT NULL,
    CONSTRAINT ck_leave_requests_status CHECK (status IN ('Pending', 'Approved', 'Rejected', 'Withdrawn')),
    CONSTRAINT ck_leave_requests_withdrawal_status CHECK (
        withdrawal_status IS NULL OR withdrawal_status IN ('Pending', 'Approved', 'Rejected')),
    CONSTRAINT ck_leave_requests_half_day_session CHECK (
        half_day_session IS NULL OR half_day_session IN ('FirstHalf', 'SecondHalf'))
);
